...
function stopGame() {
    ...
    var btn = document.getElementById("btnStart");
    startEnabled(btn); // enable the start button since the game reset
    var btn = document.getElementById("btnStop");
    btnDisabled(btn); // disable the stop button since the game is stopped
    var showPlayer = document.getElementById("showPlayer")
...
// this function will return the actives player's avatar
...
// whenever a win is detected the correspoinding function will 
// call this function to produce the following winning process
...
// these functions will produce game sounds depending on the occation
...
function checkWinCon2(info,squareArray) {
    ...
    // do appear in the array, record their 
    // avatars and compare a    ll 3 for win cons
    for (var i in info) {
...
function checkWinCon6(info,squareArray) {
    ...
    // iterate through the growing array during
    // gametime searching for the existence of
    // index 2, index 5 and index 8 and once they
    // do appear in the array, record their 
    // avatars and compare all 3 for win cons
    for (var i in info) {
        if (info[i].charAt(0) == "2") {
...
function checkWinCon7(info,squareArray) {
    ...
    // iterate through the growing array during
    // gametime searching for the existence of
    // index 0, index 4 and index 8 and once they
    // do appear in the array, record their 
    // avatars and compare all 3 for win cons
    for (var i in info) {
        if (info[i].charAt(0) == "0") {
...
function checkWinCon8(info,squareArray) {
    ...
    // iterate through the growing array during
    // gametime searching for the existence of
    // index 2, index 4 and index 6 and once they
    // do appear in the array, record their 
    // avatars and compare all 3 for win cons
    for (var i in info) {
        if (info[i].charAt(0) == "2") {
...
function square1Animate() {
    var activePlayer = document.getElementById("showPlayer").innerHTML;
    if (activePlayer != "Game Stopped") { // if games has not yet started prevent avatar placement
        var square = "0"; // identify the square selected
        // check if the proposed square is valid
        var verdict = recordMoves(square);
        if (verdict == undefined) { // if verdict is empty than the swaure is unoccupied
            ...
            if (paintAvatar ==  "O") { // call function to animate O
                animateO(selected);
